...
class PlotterGUI(QWidget):
    def __init__(self, settings):
        ...
        # Flag which determines if need to handle multiple sensors
        ...
        if self.counter % self.settings["sample_data_mod"] == 0:
            ...
            if "x_loc" in data and "y_loc" in data:
                ...
                if x is not None and y is not None:
                    ...
                    # # TODO: Testing, will be removed later
                    # sensor_types = ['IMU', 'GoDirect']
                    # rand_sensor = rand.randint(0, 1)
                    # sensor_type = sensor_types[rand_sensor]
                    sensor_type = data["sensor_type"]
                    if "count" in data:
                        id = data["count"]
                    else:
                        id = -1
                    self.graph.add_point(x, y, sensor_type)
                    self.scroll.setText(f"{x:.4f}, {y:.4f}")
    ...
    def reset(self):
        ...
        if self.multi_sensor:
            self.graph.imux_queue.clear()
            self.graph.imuy_queue.clear()
